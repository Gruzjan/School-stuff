...
//znajduje ksiazki o danym autorze
if(isset($_POST['btnAuthor'])){
    
    $author = $_POST['author'];

    //stworz i wykonaj zapytanie
    $query = "SELECT * FROM ksiazki WHERE Autor LIKE '$author'";
    $statement = mysqli_query($connection, $query);

    //wypisz w tablicy
    echo "<div style='margin-right: 150px;'><table class='text-center'><tr class='text-orange'><th>Tytuł</th></tr>";
    while ($row = (mysqli_fetch_array($statement))){
        echo "<tr class='text-yellow'><td>{$row['Tytul']}</td><tr>";
    }
    echo "</table></div>";
}
...
<body class="text-center">
    ...
    <div>
        <form action="" method="post" class="form-search align-middle border border-warning rounded">
            <h1 class="h3 mb-3 text-yellow font-weight-normal">Znajdź książki autora.</h1>
            <p>Wystarczy tylko imie i nazwisko.</p>
            <input type="text" class="input-lg form-control" name="author"  placeholder="Imię i nazwisko"  required><br>

            <button class="btn btn-lg btn-primary btn-block" type="submit" name='btnAuthor'>Wyszukaj</button>
        </form>
    </div>
