...
if(isset($_POST['btnAddCopy'])){
    ...
    //1 jesli checkobox byl zaznaczony, 0 jesli nie byl
    ...
    //wprowadz do bazdy, poinformuj o wynikach
    $query = "INSERT INTO egzemplarzksiazki(NrInwentarzowy, IdKsiazki, CzyDost) VALUES ('{$addNum}', '{$addBookId}', '{$addAvail}')";
    $statement = mysqli_query($connection, $query);
    if($statement){
...
<body>
    ...
    <div>
        <form action="" method="post" class="form-add align-middle border border-warning rounded">
            <h1 class="h3 mb-3 text-yellow font-weight-normal">Dodaj nowy egzemplarz</h1>
            <p>Dodaj nowy egzemplarz do książki.</p>

            <select class="form-control" name="addBookId">
                <?php
                    include "connect.php";
                    $connection = mysqli_connect($dbHost, $dbUser, $dbPassword, $dbName);
                    $statement = mysqli_query($connection, "SELECT * FROM ksiazki");
                    while ($list=mysqli_fetch_array($statement)){
                    ?>
                        <option  value="<?php echo $list['Id'];?>"> <?php echo ($list['Autor'] . ' ' . $list['Tytul']);?></option>
                    <?php
                    }
                mysqli_close($connection);
                ?>
            </select>
            <input type="text" class="input-lg form-control" name="addNum"  placeholder="Nr inwentarzowy" required><br>

            <div class="checkbox mb-3">
                <label>
                    <input type="checkbox" name="addAvail"> Czy dostępny
                </label>
            </div>

            <button class="btn btn-lg btn-primary btn-block" type="submit" name='btnAddCopy'>Dodaj</button>
        </form>
    </div>
