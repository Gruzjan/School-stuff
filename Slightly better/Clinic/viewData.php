...
$patientId = $_POST['btnViewData'];

//find patient
$query = "SELECT * FROM pacjenci WHERE id = '$patientId'";
$statement = mysqli_query($connection, $query);
...
<html lang="pl">
  ...
  <body class='text-center'>
    ...
    <main role="main">
        ...
        <?php if($statement){ ?>
            <h1 class='header'>Patient's personal data</h1>

            <div class="text-center table-center">
            
                <table class='text-center table table-striped'><thead><tr class="text-red"><th scope="col">Name</th><th scope="col">Surname</th><th scope="col">City</th><th scope="col">PESEL</th><th scope="col">Birth date</th><th scope="col">Sex</th></tr></thead><tbody>
                <?php
                    $row = mysqli_fetch_array($statement);
                    echo "<tr class='text-blue'><td>{$row['imie']}</td><td>{$row['nazwisko']}</td><td>{$row['miasto']}</td><td>{$row['pesel']}</td><td>{$row['dataUr']}</td><td>{$row['plec']}</td></tr>";
                ?>
                </thead></table>
            </div>
        <?php }?>
