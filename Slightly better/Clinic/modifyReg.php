...
if(isset($_POST['btnModify'])){
    if($modifyName != "" || $modifySurname != ""){
        $forbidden = "/['1234567890!^£$%&*()}{@#~?><>,|=+¬-]/";
    
        //check if name or surname contains special chars
        if(preg_match($forbidden, $modifyName) || preg_match($forbidden, $modifySurname)){
            echo "<div class='alert alert-fixed alert-danger' role='alert'>Name and surname must not contain special characters</div>";
        }
        //set query depending on filled fields
        $query = "UPDATE rejestratorzy SET ";
...
if(isset($_POST['btnChange'])){
    ...
    if($pass == "" || $passNew == "" || $passNewA == ""){
        ...
        $query = "UPDATE rejestratorzy SET haslo = '$passNew' WHERE login = '$login'";
        $statement = mysqli_query($connection, $query);
        if($statement){
...
<html lang="pl">
  ...
  <body class='text-center'>
    ...
    <main role='main'>
        <div class='left'>
            <!-- form data -->
            <h1 class="header font-weight-normal">Change data</h1>
            <form action="" method="post" class="form-modify border border-danger rounded">
                <p class="text-black">At least one field is required.</p>
                ...
                <button class="btn btn-lg btn-primary btn-block" type="submit" name="btnModify" value="<?php echo $_POST['btnAccountModify']?>">Modify</button>
            </form>
        </div>
        <div class='right'>
            <!-- form password-->
            <h1 class="header font-weight-normal">Change password</h1>
            <form action="" method="post" class="form-modify border border-danger rounded">
