...
//wyswietl klase
$query = "SELECT pacjenci.imie, pacjenci.nazwisko, pacjenci.dataUr, historia.rozpoznanie FROM pacjenci JOIN wizyty ON wizyty.pacjentId = pacjenci.id JOIN historia ON historia.wizytaId = wizyty.id WHERE wizyty.lekarzId = '$id'";
$statement = mysqli_query($connection, $query);
...
<html lang="pl">
  ...
  <body class='text-center'>
    ...
    <main role="main">
        ...
        <?php if($statement){ ?>
            <table class='text-center table table-striped'><thead><tr class="text-red"><th scope="col">#</th><th scope="col">Name</th><th scope="col">Surname</th><th scope="col">Birth date</th><th scope="col">Diagnosis</th></tr></thead><tbody>
            <?php $count = 0; while ($row = mysqli_fetch_array($statement)){
                $count++;
                echo "<tr class='text-blue'><th scope='row' class='text-red'>$count</th><td>{$row['imie']}</td><td>{$row['nazwisko']}</td><td>{$row['dataUr']}</td><td>{$row['rozpoznanie']}</td></tr>";
            }?>
            </thead></table>
        <?php }?>
        </div>
